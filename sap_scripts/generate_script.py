...
def cret_vkp2_script(file_path, username, password, transaction, skus, store, start_date, end_date, export_path, filename):
   vbs_script = f'''
If Not IsObject(application) Then
   Set SapGuiAuto  = GetObject("SAPGUI")
   Set application = SapGuiAuto.GetScriptingEngine
...
'''
   ...
   # Loop through SKUs and add multiple entries for each SKU
...
session.findById("wnd[0]/usr/ctxtS_DATUM-HIGH").text = "{end_date}"
session.findById("wnd[0]/tbar[1]/btn[8]").press 'execution f9
session.findById("wnd[0]").sendVKey 149
session.findById("wnd[0]").sendVKey 149
session.findById("wnd[0]").sendVKey 0
session.findById("wnd[0]/tbar[1]/btn[26]").press 'normal view
session.findById("wnd[0]/tbar[1]/btn[38]").press 'grid control
...
'''
   ...
   # Save the script to the specified file path
   with open(file_path, 'w') as file:
      file.write(vbs_script)
